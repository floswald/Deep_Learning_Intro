...
```math
\begin{aligned}
\text{oscillatory: }f_1(x,w,c) &= \cos \left(2 \pi w_1 + \sum_{i=1}^d c_i x_i \right) \\
...
\text{continuous: }f_5(x) &= \exp \left( -\sum_{i=1}^d c_i^2 \cdot |x_i - w_i| \right) \\
...
function f(id,x1,x2,w,c)
	x = [x1,x2]
	if id == :oscillatory
		cos(2π*w[1] + dot(c,x))
	elseif id == :product_peak
		prod( [c[i]^(-2) + (x[i] - w[i])^2 for i in 1:length(x)] )^(-1)
	elseif id == :corner_peak
		(1 + dot(c,x))
	elseif id == :gaussian
		exp( -dot( c.^2, (x .- w).^2 ) )
	elseif id == :continuous
		exp( -dot( c.^2, abs.(x .- w) ) )
	elseif id == :discontinuous
		((x[1] > w[1]) | (x[2] > w[2])) ? 0.0 : exp(dot(c,x))
	else
		error("no valid key supplied")
	end
end
...
begin
	npoints = 50
	xs = range(0,1,length=npoints)
end

# ╔═╡ 9519cd4e-64fb-4fdc-b824-362e49fe9fef
coefs = [0.3,0.7]

# ╔═╡ 66f46710-f753-4c27-8ece-0c59a35a5bd8
# weights, coefficients
cparams = Dict(:oscillatory => ([0.5],[5 , 5]),
	:product_peak => ([0.5,0.8],[1,1]),
	:corner_peak => ([0.5,0.7],coefs),
	:gaussian =>([0.4,0.4],[1,1]),
	:continuous => ([0.5, 0.5] ,coefs),
	:discontinuous => ([0.5, 0.3] ,coefs))

# ╔═╡ 9065ec6f-513c-4310-acb5-e0a9bac80ced
function plotfs(x,cparams)
	Dict(
		k => plot(x,x,[f(k,xs1,xs2,v[1],v[2]) for xs1 in x, xs2 in x], st = :surface)
		for (k,v) in cparams)
...
di = plotfs(xs,cparams)

# ╔═╡ f8a081f4-06a7-4899-aba8-207c5ab70bb6
di[:product_peak]

# ╔═╡ ec3fc8e8-a09b-4db3-8248-fcde337f53a4
di[:gaussian]
...
* sample $n$ points
* evaluate one of Genz' functions above
...
# ╔═╡ cb7f0578-1c74-455d-ba8c-b6aef15929df
sampler(:discontinuous, 10, cparams)
...
function train_ns(cparams::Dict;ns = (10,50,100,500), modelbuilder)
	...
	errors = OrderedDict(
		k => OrderedDict(
			n => pred_error(k,cparams,models[k][n]) for n in ns)
		for (k,v) in cparams)
...
function plot_errors(errors::OrderedDict)
	...
	ix = 1:length(errors[:oscillatory])
	xns = collect(keys(errors[:oscillatory]))
	...
	for (id,errs) in errors
		...
		p = plot(ix, ymean,label = id == :continuous ? "mean" : "",xticks = (ix,xns), title = "F: $(id)",xlabel = "sample size", markersize = 4, markershape = :circle, ylims = (-0.1,Inf))
		plot!(p,ix,ymax,label = id == :continuous ? "max" : "", markersize = 4, markershape = :circle)
